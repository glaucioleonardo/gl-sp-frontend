...
class Core {
    ...
    retrieve(listName, fieldsArray = [], baseUrl) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                return yield pnp.sp.configure(SpCore.config, base).web.lists.getByTitle(listName).items.select(fields).getAll();
            }
            catch (reason) {
    ...
    retrieveSingle(listItemId, listName, fieldsArray = [], baseUrl) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const base = baseUrl == null ? SpCore.baseUrl : baseUrl;
            try {
                return yield pnp.sp.configure(SpCore.config, base).web.lists.getByTitle(listName).items.getById(listItemId).select(fields).get();
            }
            catch (reason) {
                const error = SpCore.onError(reason);
                SpCore.showErrorLog(reason);
